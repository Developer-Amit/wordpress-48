...
if (!class_exists('G5PlusFramework_Portfolio')) {
    class G5PlusFramework_Portfolio
    {
        function __construct()
        {
            ...
            add_filter( 'pre_get_posts',array($this, 'tag_add_custom_types'));
        ...
        function tag_add_custom_types( $query ) {
            if( is_tax('portfolio-tag') ) {
                $query->set( 'portfolio-tag', get_query_var( 'portfolio-tag' ));
                return $query;
            }
            if( is_tax(G5PLUS_PORTFOLIO_CATEGORY_TAXONOMY) ) {
                $query->set( G5PLUS_PORTFOLIO_CATEGORY_TAXONOMY, get_query_var( G5PLUS_PORTFOLIO_CATEGORY_TAXONOMY ));
                return $query;
            }
        }
        ...
        function register_tag_taxonomies() {
            $labels = array(
                'name' => _x( 'Tags', 'taxonomy general name','g5plus-megatron' ),
                'singular_name' => _x( 'Tag', 'taxonomy singular name','g5plus-megatron' ),
                'search_items' =>  esc_html__( 'Search Tags','g5plus-megatron' ),
                'popular_items' => esc_html__( 'Popular Tags','g5plus-megatron' ),
                'all_items' => esc_html__( 'All Tags' ,'g5plus-megatron'),
                'parent_item' => null,
                'parent_item_colon' => null,
                'edit_item' => esc_html__( 'Edit Tag','g5plus-megatron' ),
                'update_item' => esc_html__( 'Update Tag','g5plus-megatron' ),
                'add_new_item' => esc_html__( 'Add New Tag','g5plus-megatron'),
                'new_item_name' => esc_html__( 'New Tag Name','g5plus-megatron' ),
                'separate_items_with_commas' => esc_html__( 'Separate tags with commas','g5plus-megatron' ),
                'add_or_remove_items' => esc_html__( 'Add or remove tags','g5plus-megatron' ),
                'choose_from_most_used' => esc_html__( 'Choose from the most used tags' ,'g5plus-megatron'),
                'menu_name' => esc_html__( 'Tags','g5plus-megatron' ),
            );

            register_taxonomy('portfolio-tag',G5PLUS_PORTFOLIO_POST_TYPE,array(
                'hierarchical' => false,
                'labels' => $labels,
                'show_ui' => true,
                ...
                'fields' => array(
                    ...
                    array(
                        ...
                        'options'  => array(
                            ...
                            'detail-09' 	=> esc_html__( 'Small video slider', 'g5plus-megatron'),
                            'detail-10' 	=> esc_html__( 'Horizontal video slider', 'g5plus-megatron'),
                            ),
                        'multiple'    => false,
                        'std'         => 'none',
                    )
        ...
        function get_portfolio_archive_template($archive_template) {
            ...
            if (( is_tax('portfolio-tag') || is_tax(G5PLUS_PORTFOLIO_CATEGORY_TAXONOMY) || is_post_type_archive(G5PLUS_PORTFOLIO_POST_TYPE) ) ) {
	            $g5plus_options = &megatron_get_options_config();
